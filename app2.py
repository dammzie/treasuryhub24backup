...
app2_blueprint = Blueprint('app2', __name__, template_folder='templates2')

@app2_blueprint.route('/application2', methods=['GET', 'POST'])
def application2():
    return render_template('application2.html')

@app2_blueprint.route('/process_file_2', methods=['POST'])
def process_file_2():
    file = request.files['file']
    if file:
        file.stream.seek(0)  # Ensure we're at the start of the file stream
        buffer = io.BytesIO(file.stream.read())  # Read the file into a BytesIO object
        df = pd.read_excel(buffer)  # Now read from the seekable stream
        df['TradeDate'] = pd.to_datetime(df['TradeDate'])
        session['uploaded_df'] = df.to_dict()  # Storing DataFrame in session
        return redirect(url_for('app2.app2success'))  # Note the 'app2.' prefix to refer to the blueprint
    else:
        flash("No file uploaded")
        return redirect(url_for('app2.application2'))  # Note the 'app2.' prefix to refer to the blueprint

@app2_blueprint.route('/app2success', methods=['GET', 'POST'])
def app2success():
    if 'uploaded_df' not in session:
        flash("No data available.")
        return redirect(url_for('app2.application2'))
    ...
    if request.method == 'POST':
        ...
        filtered_data = filtered_date_data[filtered_date_data['Reason'].isin(['FX Option Buy-NIH', 'FX Option Sell - NIH'])]
        ...
        for index, row in filtered_data.iterrows():
            ...
            maturity_date = row['MaturityDate'].strftime("%dth of %B %Y")
            ...
            if row['Currency1'] == 'USD':
                ...
                currency = row['Currency']  # You can choose either currency1 or currency2 as default
            ...
            if row['Currency1'] == 'USD' and row['Reason'] == 'FX Option Buy-NIH' :
                Notional = row['Principal1']
                Notional_value = abs( Notional/ 1000000)  # Convert to millions and ensure positive value
                ratediva = row['StrikeRate']
                ratediv = round(ratediva, 4)
                rate2 = row['StrikeRate']*0.1
                ratedivb = row['StrikeRate'] + rate2
                ratediv2 = round(ratedivb, 4)
                answer1 = abs( Notional_value/ ratediv)
                answerr1 = round(answer1, 1)
                answer1b = abs( Notional_value/ ratediv2)
                answerr1b = round(answer1b, 1)
            elif row['Currency1'] == 'USD' and row['Reason'] == 'FX Option Sell - NIH' :
                Notional = row['Principal1']
                Notional_value = abs( Notional/ 1000000)  # Convert to millions and ensure positive value
                ratediva = row['StrikeRate']
                ratediv = round(ratediva, 4)
                ...
                answer1b = abs( Notional_value/ ratediv2)
                answerr1b = round(answer1b, 1)
            elif row['Currency2'] == 'USD' and row ['Reason'] == 'FX Option Sell - NIH':
                Notional = row['Principal2']
                Notional_value = abs( Notional/ 1000000)  # Convert to millions and ensure positive value
                ratediva = row['StrikeRate']
                ratediv = round(ratediva, 4)
                ...
                answer1b = abs( Notional_value/ ratediv2)
                answerr1b = round(answer1b, 1)
            elif row['Currency2'] == 'USD' and row ['Reason'] == 'FX Option Buy-NIH':
                Notional = row['Principal2']
                Notional_value = abs( Notional/ 1000000)  # Convert to millions and ensure positive value
                ratediva = row['StrikeRate']
                ratediv = round(ratediva, 4)
            ...
            if row['Reason'] == 'FX Option Buy-NIH':
                ratediva = row['StrikeRate']
                ratediv = round(ratediva, 4)
                rate = 'above ' + str(ratediv) + ' (Buy)'
            elif row['Reason'] == 'FX Option Sell - NIH':
                ratediva = row['StrikeRate']
                ratediv = round(ratediva, 4)
                rate = 'below ' + str(ratediv) + ' (Sell)'
            else:
                rate = ''
        ...
        <b>Start Date:</b>&nbsp {trade_date}<br/>
        ...
        <b><font size="11">Subject: Designation of {currency} {Notional_value}M FX Forward Option Contract as Net Investment Hedge</font></b><br/>
        <br/>
        ...
        The objective of this designation is to mitigate the currency exchange risk arising from the translation of the net assets of our subsidiaries. By designating this FX forward Option contract as a hedge, we aim to protect the value of our net investment in these subsidiaries from adverse foreign exchange movements.<br/><br/>

        <b>Hedge Relationship:</b> <br/>
        The FX forward Option contract is designated as a hedge of the first<font color="#007da5"> {currency} {Notional_value}M</font> million of net assets held in subsidiaries. The fair value changes of the FX forward option will offset the gains or losses of the net investment in subsidiaries attributable to changes in foreign exchange rates<font color="#007da5"> {rate}</font>.<br/><br/>
        ...
        The designated hedging instrument is a<font color="#007da5"> {currency} {Notional_value}M</font> FX forward Option contract entered into with <font color="#007da5">{counterparty}</font>. The contract has a start date of <font color="#007da5">{trade_date}</font> and a maturity date of <font color="#007da5">{maturity_date}</font>.<br/><br/>

        <b>Risk Management Objective and Strategy:</b><br/>
        The risk management objective is to reduce the volatility in the consolidated financial statements resulting from changes in foreign exchange rates. This strategy involves using the FX forward Option contract to mitigate the potential adverse impact of currency fluctuations on the value of our net investment in subsidiaries.<br/><br/>

        <b>Prospective Hedge Effectiveness Test:</b><br/>

        To demonstrate the prospective hedge effectiveness, let's consider an opening GBP/USD exchange.

        <br/>Change in Hedging Instrument Value: The designated hedging instrument value is<font color="#007da5"> {Notional_value}M</font>. As the GBP/USD exchange rate moves from <font color="#007da5">{ratediv}</font> to <font color="#007da5">{ratediv2}</font>, the change in the value of the FX forward Option contract denominated in GBP would be: 
        <br/><font color="#007da5">Variable 1 =( {currency} -{Notional_value}M /{ratediv}) - ( {currency} -{Notional_value}M /{ratediv2}) 
        <br/>= -{answerr1}M - (-{answerr1b})M = -{finall}M</font>
        ...
        <br/><font color="#007da5">Variable 2 =( {currency} {Notional_value}M /{ratediv}) - ( {currency} {Notional_value}M /{ratediv2}) 
        <br/>= {answerr1}M - {answerr1b}M = {finall}M</font>
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name='NIH_Option_Buy_Sell_Report.pdf',
            mimetype='application/pdf'
        )
        
    return render_template('app2success.html')
