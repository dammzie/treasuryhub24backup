...
/*!*******************************************!*\
  ...
  ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
    ...
    function renderChart(data, filterType) {
        ...
        if (filterType === "all") {
            // Show both IHB and Cash Diary Amount series
            series = [
                {
                    name: 'In-House Position',
                    type: 'line',
                    data: data.seriesIHB,
                    color: '#008FFB'
                },
                {
                    name: 'External Deals',
                    type: 'line',
                    data: data.seriesCash,
                    color: '#00E396'
                },
                {
                    name: 'Net Amount',
                    type: 'line',
                    data: data.seriesDiscrepanczz, // Add this line
                    color: '#FF4560' // Choose an appropriate color
                }
            ];
        } else if (filterType === "positive") {
            // Show only Cash Diary Amount series
            series = [
                {
                    name: 'External Deals',
                    type: 'line',
        ...
        const taskOverviewChartOption = {
            ...
            // Add any other configuration you need for the chart
        };
    
        // Check if the chart instance already exists
        if (chart) {
            // Update the existing chart's options and redraw
            chart.updateOptions(taskOverviewChartOption);
        } else {
            // Create a new chart instance
            chart = new ApexCharts(document.querySelector("#task-overview-chart"), taskOverviewChartOption);
            chart.render();
        }
    
        // Display total, positive, and negative counts
        document.querySelector("#total-count").textContent = data.totalCount;
        document.querySelector("#positive-count").textContent = data.positiveCount;
        document.querySelector("#negative-count").textContent = data.negativeCount;
    }
    
    document.querySelectorAll('input[name="groupRadioExample"]').forEach(radio => {
        radio.addEventListener('change', function() {
            if (this.checked) {
                fetch('/data').then(response => response.json()).then(data => {
                    renderChart(data, this.value); // Pass the value of the selected radio button
                });
            }
        });
    });
    
    function calculateDaysRemaining(valueDateString) {
        const valueDate = new Date(valueDateString);
        const currentDate = new Date();
    
        const diffInTime = valueDate.getTime() - currentDate.getTime();
    ...
    function populateTable(data) {
        ...
        data.forEach(row => {
            const daysRemaining = calculateDaysRemaining(row.ValueDate);
              // Determine the styled HTML based on days remaining
            let styledDaysRemainingHtml;
            if (daysRemaining <= 5) {
                styledDaysRemainingHtml = `<div class="flex items-center"><span class="badge-dot bg-red-500"></span><span class="ml-2 rtl:mr-2 capitalize font-semibold text-red-500">${daysRemaining} days left</span></div>`;
            } else if (daysRemaining === 10) {
                styledDaysRemainingHtml = `<div class="flex items-center"><span class="badge-dot bg-yellow-500"></span><span class="ml-2 rtl:mr-2 capitalize font-semibold text-yellow-500">${daysRemaining} days left</span></div>`;
            } else {
                styledDaysRemainingHtml = `<div class="flex items-center"><span class="badge-dot bg-green-500"></span><span class="ml-2 rtl:mr-2 capitalize font-semibold text-green-500">${daysRemaining} days left</span></div>`;
            }

            // Function to format the number in Millions (M) or Thousands (K)
            function formatNumber(num) {
                if (Math.abs(num) >= 1000000) {
                    // If the number is in millions
                    return (num / 1000000).toFixed(1) + 'M';
            ...
            // Compare values and determine the label
            let statusContent, statusClasses;
            const discrepancy = row.Discrepanczz / 1000000;
            if (discrepancy > 5) { // Discrepancy above 5 million
                statusContent = 'Error';
                statusClasses = 'text-red-600 bg-red-100 dark:text-red-100 dark:bg-red-500/20';
            } else if (discrepancy > 1) { // Discrepancy between 1 million and 5 million
                statusContent = 'Have a Look';
                statusClasses = 'text-amber-600 bg-amber-100 dark:text-amber-100 dark:bg-amber-500/20';
            } else { // Discrepancy below 1 million
                statusContent = 'Good';
                statusClasses = 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-100';
            }
            ...
            tr.innerHTML = `
                ...
                <td class="font-bold">${row.ValueInMillionsIHB}M</td>
                <td class="font-bold">${row.ValueInMillionsCash}M</td>
                <td class="font-bold">${formatNumber(row.Discrepanczz)}</td>
                <td>
                <div class="inline-flex items-center px-2 py-1 border border-gray-300 rounded-full">
                    ...
                    <span class="ml-1 rtl:mr-1 whitespace-nowrap">${row.ValueDate}</span>
                </div>
                </td>
                <td>${styledDaysRemainingHtml}</td>

                <td>
                <div class="tag ${statusClasses} rounded border-0">
                    ${statusContent}
                </div>
                </td>
                <td>
                <div class="tag rounded-md font-bold cursor-pointer select-none text-gray-900 dark:text-gray-100">View All</div>
                </td>
    ...
    fetch('/data').then(response => response.json()).then(data => {
        renderChart(data, 'all'); // Existing function to render chart
        populateTable(data.tableData); // New function to populate table
    });
