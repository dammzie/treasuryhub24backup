...
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
    ...
    let combinedChart; // Hold the combined chart instance

    // Function to render combined line chart
    function renderCombinedChart(chartId, debitData, creditData, debitKey, creditKey) {
        let debitSeries = {
            name: 'Crebit',
            data: debitData.map(item => item[debitKey])
        };
    
        let creditSeries = {
            name: 'Dedit',
            data: creditData.map(item => item[creditKey])
        };
    
        // Helper function to format tooltip value
        function formatTooltipValue(value, isGBP, currency) {
            let formattedValue = (value / 1e6).toFixed(2) + 'M';
            let currencySymbol = isGBP ? 'GBP' : currency;
            return formattedValue + ' ' + currencySymbol;
        }
    
        let options = {
            series: [debitSeries, creditSeries],
            chart: {
                type: 'line',
                height: 400,
                toolbar: {
                ...
                offsetY: 0,
                offsetX: 0,
                // Adjust margin to provide more space for x-axis labels
                margin: {
                    bottom: 100
                }
            },
            ...
            yaxis: {
                labels: {
                    formatter: function (val) {
                        return (val / 1e6).toFixed(2) + 'M'; // This will format the y-axis values as millions
                    }
                }
            },
            tooltip: {
                ...
                y: {
                    formatter: function (value, { series, seriesIndex, dataPointIndex }) {
                        let isGBP = document.getElementById('currency-select').value === 'GBP';
                        let currency = isGBP ? 'GBP' : (seriesIndex === 0 ? debitData[dataPointIndex].Receive_currency : creditData[dataPointIndex].Pay_currency);
                        return formatTooltipValue(value, isGBP, currency);
                    }
                },
                x: {
                    show: true
            ...
            xaxis: {
                categories: debitData.map(item => item.To_debit),
                labels: {
                    rotate: -45,
                    trim: true,
                    minHeight: 120, // Adjust if necessary
                    style: {
...
// Helper function to find the entity with the maximum value
function findEntityWithMaxValue(transactions, valueKey) {
    return transactions.reduce((max, item) => {
        const value = Number(item[valueKey]);
        if (!max || value > Number(max[valueKey])) {
            return item;
        }
        return max;
    }, null);
...
function generateSummary(totalCredit, totalDebit, creditData, debitData) {
    // Find the entities with the maximum credit and debit
    let maxCreditEntity = findEntityWithMaxValue(creditData, 'Pay_GBP_equiv');
    let maxDebitEntity = findEntityWithMaxValue(debitData, 'Receive_GBP_equiv');

    // Construct the summary message
    const summaryMessage = `The total credit amount is GBP ${totalCredit.toFixed(2)}M and the total debit amount is GBP ${totalDebit.toFixed(2)}M. ` +
                           `The entity with the most credits is ${maxCreditEntity.From_credit || 'N/A'} with GBP ${(maxCreditEntity.Pay_GBP_equiv / 1e6).toFixed(2)}M. ` +
                           `The entity that sent out the most is ${maxDebitEntity.To_debit || 'N/A'} with GBP ${(maxDebitEntity.Receive_GBP_equiv / 1e6).toFixed(2)}M.`;
    return summaryMessage;
}
// Function to fetch and render chart data
function fetchAndRenderData() {
    fetch('/gssm/pivot-data')
        .then(response => response.json())
        .then(data => {
            // Calculate total credit, ensuring we only sum valid numbers
            let totalCredit = data.credit.reduce((acc, cur) => {
                const value = Number(cur.Pay_GBP_equiv);
                return acc + (isNaN(value) ? 0 : value);
            }, 0) / 1e6; // Convert to millions

            // Calculate total debit, ensuring we only sum valid numbers
            let totalDebit = data.debit.reduce((acc, cur) => {
                const value = Number(cur.Receive_GBP_equiv);
                return acc + (isNaN(value) ? 0 : value);
            }, 0) / 1e6; // Convert to millions

            // Generate the summary text including the entities with the most credit and debit
            const summaryText = generateSummary(totalCredit, totalDebit, data.credit, data.debit);
            // Update the HTML with the summary text
            document.getElementById('data-summary').textContent = summaryText;

            // Update the HTML with the totals, checking for NaN before setting text
            document.getElementById('total-credit').textContent = isNaN(totalCredit) ? 'N/A' : `${totalCredit.toFixed(2)}M`;
            document.getElementById('total-debit').textContent = isNaN(totalDebit) ? 'N/A' : `${totalDebit.toFixed(2)}M`;

            // If you need to render a chart or other data visualizations, call that function here
            // Assuming combinedChart and renderCombinedChart are defined elsewhere
            combinedChart = renderCombinedChart("#combined-chart", data.debit, data.credit, 'Receive_GBP_equiv', 'Pay_GBP_equiv');
        })
...
document.getElementById('currency-select').addEventListener('change', function() {
    let isGBP = this.value === 'GBP';
    let debitCurrencyKey = isGBP ? 'Receive_GBP_equiv' : 'Receive_LC';
    let creditCurrencyKey = isGBP ? 'Pay_GBP_equiv' : 'Pay_LC';
    fetch('/gssm/pivot-data')
        .then(response => response.json())
        .then(data => {
            // Calculate and update totals based on selected currency, excluding zero values
            let totalCredit = data.credit
                .filter(item => item[creditCurrencyKey] !== 0)
                .reduce((acc, cur) => acc + cur[creditCurrencyKey], 0) / 1e6;

            let totalDebit = data.debit
                .filter(item => item[debitCurrencyKey] !== 0)
                .reduce((acc, cur) => acc + cur[debitCurrencyKey], 0) / 1e6;

            document.getElementById('total-credit').textContent = `${totalCredit.toFixed(2)}M`;
            document.getElementById('total-debit').textContent = `${totalDebit.toFixed(2)}M`;

            // Update the chart series
            combinedChart.updateSeries([
                { name: 'Credit', data: data.debit.map(item => item[debitCurrencyKey]).filter(val => val !== 0) },
                { name: 'Debit', data: data.credit.map(item => item[creditCurrencyKey]).filter(val => val !== 0) }
            ]);
        });
});
